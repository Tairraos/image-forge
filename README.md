...
<p align="center">
  <img alt="version" src="https://img.shields.io/badge/version-0.2.52-9B7BEE?style=flat-square">
...
- 失败任务的完整错误信息会向下撑开顶部状态框，结果预览区在剩余空间内弹性缩小。
- 所有滚动条的轨道和滑块统一使用一秒淡入淡出；滚动或鼠标靠近边缘时显示完整浅灰轨道，停止滚动后自动隐藏。
- API 源卡片增加高度以完整展示四行配置内容，脱敏密钥行不再显示 `Key:` 前缀。
- 模板提示词和参考图气泡改为紧邻鼠标显示并挂载到窗口层，避免被弹窗裁剪；空间不足时自动向上展开，长提示词最多显示 12 行后内部滚动。
- API 源与模板维护的导入导出按钮使用一致图标：导入为向下进入托盘，导出为向上离开托盘。

### v0.2.47

- 模板维护列表新增持久化上下排序；标题点击直接查看模板，并在悬浮时预览完整提示词。
- 参考图数量支持悬浮查看 `64×64` 缩略图；操作列移除“查看”，列表最多显示 12 条且表头固定。
- 全局滚动条统一为细样式，仅在滚动中或鼠标靠近滚动区域边缘时显示，停止滚动后自动隐藏。

### v0.2.46

- API 源批量导入会忽略随机 ID，对名称、类型、地址、Key、代理、模型和启用状态完全相同的配置去重，并弹窗显示导入与重复数量。
- 模板 ZIP 导入会同时检查现有模板和同批模板，标题、正文和参考图完全相同时跳过，并弹窗显示导入与重复数量。

### v0.2.45

- API 源导入文本框固定为 12 行高度，长 JSON 内容改为在文本框内部滚动。

### v0.2.44

- API 源卡片调整为五行结构：标题、模型类型、模型名称、脱敏 Key、右对齐操作按钮。

### v0.2.43

- API 源卡片改为单行横向列表，固定宽度 `180px`，溢出时显示横向滚动条，排序操作改为左右箭头。
- 卡片按标题、模型类型、模型名称、脱敏 Key、操作按钮五行排列；Key 只显示前后各 6 个字符。
