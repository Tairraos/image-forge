...
> **重要**：浏览器 localStorage 按域名+端口隔离。同步页面必须和 Web 应用在同一端口（1421），否则数据写入后不可见。

**方式一：控制台导入（推荐，无需停 Vite）**
```bash
pnpm sync:web
```
复制输出的 JS 代码，在 Web 应用的浏览器控制台（F12 → Console）粘贴执行。
...
# 2. 运行同步
pnpm sync:web:serve
# 3. 浏览器打开 http://localhost:1421，点击「导入数据」
