...
Skill 是不依赖外部脚本的 Markdown 包，目录规则与 Codex 一致：`skills/<skill-name>/SKILL.md`。数据模型为 `SkillEntry`，`skills.json` 保存索引元数据，正文和可选参考文档保存在 Skill 包内：
...
拖入本地目录时，后端会查找其中的 `SKILL.md` 或 `skill.md`，保存时复制同级 `references/*.md` 到 Skill 包。执行规划时按文件名排序加载这些 Markdown，并使用 `<skill_reference>` 标记附加到 Skill 内容；只读取 Markdown 文件，不执行脚本或其他文件。
