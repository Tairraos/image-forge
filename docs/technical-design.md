...
API 源卡片按配置数组顺序横向排列，固定宽度为 `180px`、最小高度为 `156px`，外层横向列表保留至少 `180px` 高度，确保四行信息和底部操作区完整显示；溢出时只允许水平滚动。左右排序按钮直接调整数组顺序。卡片使用标题、模型类型、模型名称、脱敏密钥、操作区五行结构，密钥不显示字段前缀，只保留前后各 6 个字符。JSON 导入导出规则如下：
